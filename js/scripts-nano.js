...
$(document).on("click", ".tz-test-submit", function(e){
...
/*	var getEmail = $(".tz-email-input").val();
    ...
    if($(this).closest('form').find(".tz-city-input").val() != undefined){
        var getCity = $(this).closest('form').find(".tz-city-input").val(); 
         if(getCity.length == 0)
        {
            var errorNameMsg = "<div class='tz-error-validate-city-msg inline-block-tz'><span>Поле 'Город' должно быть заполнено.</span></div>" 
...
/*    
	...
	if(getName.length == 0)
	{
		...
		var errorNameMsg = "<div class='tz-error-validate-name-msg no_valid'><span>Поле 'Имя' должно быть заполнено.</span></div>";
	...
	if(getPhone.length == 0)
	{
		...
		var errorPhoneMsg = "<div class='tz-error-validate-phone-msg no_valid'><span>Поле 'Телефон' должно быть заполнено.</span></div>";
	...
	else
	{
		...
		else
		{
			...
			var errorPhoneValidMsg = "<div class='tz-error-validate-name-msg no_valid'><span>Приемлемо использование только: <br> (цифры,  скобки () и тире) </span></div>";
			$(".tz-phone-input").addClass("tz-email-input-error");
	...
	if(getEmail.length > 0 || getEmail.length == 0)
	{
		...
		else
		{
			...
			var errorMailMsg = "<div class='tz-error-validate-msg no_valid'><span>Поле 'E-mail' должно содержать символ '@'.</span></div>";
    ...
    if(inputCityIsset == "yes"){
	    if(nameValidate == true && emailvalidate == true && phoneValidate == true && cityValidate == true)  
	    {
		    ...
		    $.post("/ajax/add_form.php", GetDataInputs, function(data){
			    //alert(data);  
        ...
        if(nameValidate == true && emailvalidate == true && phoneValidate == true )  //&& cityValidate == true
        {
            ...
            $.post("/ajax/add_form.php", GetDataInputs, function(data){
            	...
            	$('.callback_box form, .callback_box > .callback_ico').hide();
            	
            	$('.ans-container p.success-msg').html(data);
            	$('.ans-container').show();
...
$(document).on('click' , '#search-category .list-cat li span' , function(){
	var category = $(this).text();
	var category_num = $(this).attr('category');
	console.log(category);
	$(this).closest("#search-category").find('span.first_menu').text(category).attr('category', category_num).append('<i class="tz-caret"></i>');
});

/*
function search_jump(category, query){

	if (parseInt(category) == 0){
		window.location.href = '/search/index.php?q=' + query;
	}
	if (parseInt(category) == 1){
		window.location.href = '/search/index.php?q=' + query + '&category=corporate';
	}
	if (parseInt(category) == 2){
		window.location.href = '/search/index.php?q=' + query + '&category=shop';
	}
	return;
}    */
...
$(document).ready(function(){
	 $('.search_block input[type="text"]').keypress(function(e){       
	   if(e.keyCode==13){
	   	   var category_num = $('#search-category span.first_menu').attr('category');
	   	   if ($(this).val().length > 0){
				//search_jump(category_num , $(this).val() );
				window.location.href = '/search/index.php?q=' + $(this).val();
	   	   } 
	   }
	 });
	 
	 $(document).on('click' , '.search_block input[type="submit"]', function(e){

	 	 e.preventDefault();
	   	 var category_num = $('#search-category span.first_menu').attr('category');
	   	 var search_q_len = $('.search_block input[type="text"]').val().length;
	   	 var search_q = $('.search_block input[type="text"]').val();

	   	 if ( parseInt(search_q_len) > 0){
				//search_jump(category_num , search_q);
				window.location.href = '/search/index.php?q=' + search_q;
	   	 } 
	 });
	 
	 $(document).on('click','.btn_start', function(){
	        var destination = $(".callback_phone_block").offset().top - 50;
	        $('html, body').animate({ scrollTop: destination }, 3000); //1100 - скорость прокрутки
	        return false; 
	 });
