...
//	$("a[name=delete-item]").live('click', function() {
	$("a[name=delete-item]").on('click', function() {
    ...
    $("a[href*=ADD2BASKET]").click(function() {
        ...
        var tz_quan = $(this).data('quant');
        var tz_count = $(this).data('count');
        var tz_needed = count;
        var rest = tz_needed / tz_count;
        
        if(parseInt(rest) != rest){
            if(tz_needed < tz_count){
                if(tz_count <= tz_quan){
                    $('.tz_alert_el').text('Кратность товара '+tz_count+', добавлено '+tz_count+' из наличия на складе.');     
                }
                else{
                    $('.tz_alert_el').text('Кратность товара '+tz_count+', добавлено '+tz_quan+' из наличия на складе.');  
                    //$('.tz_alert_el').text('В наличии '+tz_quan+' от кратности');   
                }    
            }
            else{
                var result = Math.ceil(rest) * tz_count;
                if(result <= tz_quan){
                    if(tz_needed == tz_quan){
                        $('.tz_alert_el').text('Товар добавлен в корзину');    
                    }
                    else{
                        result = result - tz_count;
                        $('.tz_alert_el').text('Кратность товара '+tz_count+', добавлено '+result+' из наличия на складе.');;                         
                    }    
                } 
                else{
                    if(tz_needed <= tz_quan){
        ...
        else
        {
            $.ajax({
                async: true,
                type: "GET",
                url: "/includes/nano/ajax/basket.add.php?"+href,
                data: "",
                beforeSend: function() {
                },
                success: function(msg) {
                    $.ajax({
                        async: true,
                        type: "GET",
                        url: '/includes/nano/ajax/basket.small.php',
                        data: "",
                        beforeSend: function() {
                        },
                        success: function(msg) {
                            
                            $("#small-basket-nano").html(msg);
    ...
    $(".btn-to-basket").live("click", function(e) {     
    	e.preventDefault();
		var link = $(this);
		var href = $(this).attr("href").substr($(this).attr("href").indexOf("?")+1);
		var id = parseInt(href.substr(href.indexOf("id=")+3));
...
//	$("div.in_basket a.close").live('click', function() {
	$("div.in_basket a.close").on('click', function() {
