...
$rsBanners = CAdvBanner::GetList($by="s_weight", $order, $arFilter, $is_filtered, "N");
...
while($arBanner = $rsBanners->NavNext(true, "f_"))
{
	$arBanners[]=array(
        ...
        'IMG_THUMB'=> CFile::ResizeImageGet($arBanner['IMAGE_ID'], array('width'=>55, 'height'=>35), BX_RESIZE_IMAGE_PROPORTIONAL, true),
		'URL'=>$arBanner['URL']
	);
    //debugArray($arBanner);
...
<?/*<div class="block711">*/?>
	<div class="slider_big" id="slider_big">
		<?foreach($arBanners as $banner){?>
			<a href="<?=$banner['URL'];?>" title="<?=$banner['NAME'];?>"><img
                    src="<?=$banner['IMG'];?>" alt="" class="banner711"
                    data-thumb="<?=$banner['IMG_THUMB']['src'];?>"/>
            </a>
		<?}?>
	</div>
<?/*</div>*/?>