		...
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
							...
							<div class="bx_filter_parameters_box_container">
								<div class="bx_filter_parameters_box_container_block">
									<div class="bx_filter_input_container">
										...
										onkeyup="this.value > 0 ? smartFilter.keyup(this) : this.value = '';"
										/>
									</div>
								</div>
								<div class="bx_filter_parameters_box_container_block">
			...
			foreach($arResult["ITEMS"] as $key=>$arItem)
			{
				...
				<div id="prop_<?=$arItem['ID']?>" class="bx_filter_parameters_box <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>active<?endif?>">
					...
					<div class="bx_filter_block">
<?
if($arItem["NAME"] == "Производитель" || $arItem["NAME"] == "Серия") echo '<div class="filter_list"><input placeholder="Я ищу..." type="text" onkeyup="filterList(this)"></div>';
?>
						<div class="bx_filter_parameters_box_container">
						...
						switch ($arItem["DISPLAY_TYPE"])
						{
							...
							default://CHECKBOXES
								...
								<?foreach($arItem["VALUES"] as $val => $ar):?>
									<?
									$color = $arItem["NAME"] == "Цвет" ? 'style="background-color:#'.$ar["URL_ID"].'"' : '';
									?>
									<label <?=$color?> data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
										<span class="bx_filter_input_checkbox">
											<input
												type="checkbox"
												value="<? echo $ar["HTML_VALUE"] ?>"
			...
			<div class="content col col-9">
				...
				<div class="catalog-sort-panel clearfix">
								...
								<div class="popup-inner">
									...
									*/?>
									...
									<ul>
										...
										<li><a href="?<?if($get_parameters):?><?=$get_parameters?><?endif;?>sort=available&order=desc" <?/*class="sort_link"*/?> data-change="aavailable_desc">По наличию</a></li>										
									</ul>
								</div>
							</div>
						</div>
					...
					<form method="GET" name="available">
						...
						<label for="styled-checkbox-2">Только в наличии</label>
					</form>
				</div>
