...
<div class="lightbox-holder">
	<div id="<?=$cntId?>_calculate" class="lightbox calculate">
        ...
        <p><?=GetMessage('CALC6_POPUP_NOTE')?></p>
...
$(function(){	
	var <?=$cntId?>_calcTable;
	<?=$cntId?>_calcTable = <?=json_encode($arCalcTable)?>;
	...
	$('#<?=$cntId?>_calc_form .button.lightbox-open').on('click',function(){
		...
		if(!$('#<?=$cntId?>_calculate .feedback_form_note').hasClass('success')){
			...
			// _calcTable = $.makeArray(<?=$cntId?>_calcTable);
			_calcTable = <?=$cntId?>_calcTable;
			...
			if(_selectedOption.length > 0){
				_structureId = _selectedOption.val();
				if(parseInt(_structureId) > 0){
				
					_calcStr = _calcTable[_structureId];

					if(Object.keys(_calcStr).length > 0){
						for(i in _calcTable[_structureId]){
							console.log(_calcTable[_structureId][i]);
							if(_y > i)
								_x = _calcTable[_structureId][i];
							else
								break;
						}
					}
