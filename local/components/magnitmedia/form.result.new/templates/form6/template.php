...
<form action="#" id="<?=$cntId?>_calc_form">
	<div class="clearfix">
		<div class="form-tablet-column">
			...
			if($arCalc[$fieldName]['STRUCTURE']):?>
				<select data-field-name="form_dropdown_<?=$fieldName?>" class="calc_field" data-calc-var="<?=$arCalc[$fieldName]['VAR']?>">
					<option value=""><?=$arCalc[$fieldName]['CAPTION']?></option>
		...
		<div class="form-tablet-column">
			...
			?>
			<div class="input-holder">
				<textarea data-field-name="form_<?=$arStructure['FIELD_TYPE']?>_<?=$arStructure['ID']?>" placeholder="<?=GetMessage('FORM6_MESSAGE')?>" class="calc_field" cols="30" rows="10"></textarea>
			</div>
			<?endif;?>
			<?/*
			$fieldName = 'work_time';
			...
			<?endif;*/?>
			<a class="button lightbox-open form-open" <?if (SITE_SERVER_NAME == "elevel.ru"):?>onclick="yaCounter1485305.reachGoal('rasschitat');return true;"<?endif;?> href="#<?=$cntId?>_calculate"><?=($arParams['BUTTON_TITLE'])?$arParams['BUTTON_TITLE']:GetMessage('CALC6_BUTTON_TITLE')?></a>
		</div>
	</div>
