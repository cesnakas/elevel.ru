...
$arCalc = array();
$strParams = "";
foreach ($arCalcSort as $field_sid)
{
...
<form action="#" id="<?=$cntId?>_calc_form">
	<div class="form-row clearfix">
		<div class="form-column">
			<?if($arCalc['producer']['STRUCTURE']):?>
			<div class="input-holder">
				<select data-field-name="form_dropdown_producer" class="calc_field" data-calc-var="x">
					<option value="<?=$arStructure['ID']?>"><?=$arCalc['producer']['CAPTION']?></option>
					<?foreach($arCalc['producer']['STRUCTURE'] as $arStructure):?>
					<option data-calc-val="<?=$arStructure['FIELD_PARAM']?>" value="<?=$arStructure['ID']?>"><?=$arStructure['MESSAGE']?></option>
					<?endforeach;?>
				</select>
			</div>
			<?endif;?>
			<?if($arCalc['column_number']['STRUCTURE']):?>
			<div class="input-holder">
				<?foreach($arCalc['column_number']['STRUCTURE'] as $arStructure):?>
				<input placeholder="<?=$arCalc['column_number']['CAPTION']?>" class="calc_field" data-field-name="form_<?=$arStructure['FIELD_TYPE']?>_<?=$arStructure['ID']?>" data-calc-var="y" type="text"/>
				<?endforeach;?>
			</div>
			<?endif;?>
		</div>
		<div class="form-column">
			<?if($arCalc['current']['STRUCTURE']):?>
			<div class="input-holder">
				<?foreach($arCalc['current']['STRUCTURE'] as $arStructure):?>
				<input placeholder="<?=$arCalc['current']['CAPTION']?>" class="calc_field" data-field-name="form_<?=$arStructure['FIELD_TYPE']?>_<?=$arStructure['ID']?>" data-calc-var="z" type="text"/>
				<?endforeach;?>
			</div>
			<?endif;?>
			<a class="button lightbox-open" <?if (SITE_SERVER_NAME == "elevel.ru"):?>onclick="yaCounter1485305.reachGoal('rasschitat');return true;"<?endif;?> href="#<?=$cntId?>_calculate"><?=($arParams['BUTTON_TITLE'])?$arParams['BUTTON_TITLE']:GetMessage('CALC_BUTTON_TITLE')?></a>
		</div>
	</div>
</form>
...
<div class="lightbox-holder">
	<div id="<?=$cntId?>_calculate" class="lightbox calculate">
		...
		<p><?=GetMessage('CALC_POPUP_NOTE')?></p>
				...
				<?=$arResult["FORM_HEADER"]?>
					...
					foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion)
					{
						...
						if ($arQuestion['STRUCTURE'][0]['FIELD_TYPE'] == 'hidden')
						{
							...
							if($FIELD_SID == "manager_email" || $FIELD_SID == "form_title" || $FIELD_SID == "page_title" || $FIELD_SID == "page_url" || $FIELD_SID == "cost"
							...
							elseif($FIELD_SID == "producer" || $FIELD_SID == "column_number" || $FIELD_SID == "current"):
							?>
...
$(function(){	
	$('#<?=$cntId?>_calc_form .button.lightbox-open').on('click',function(){
		...
		if(!$('#<?=$cntId?>_calculate .feedback_form_note').hasClass('success')){
			...
			_z = parseInt($('#<?=$cntId?>_calc_form input[data-calc-var=z]').val());
			...
			if(_res == 0 || isNaN(_res))
			{
				...
				$('#<?=$cntId?>_producer').html(_xVal);
				$('#<?=$cntId?>_current').html(_z);
				$('#<?=$cntId?>_column_number').html(_y);
