...
<form action="#" id="<?=$cntId?>_calc_form">
	<div class="form-row clearfix">
		<div class="form-column">
			...
			<?endif;?>
			<?if($arCalc['lines_number']['STRUCTURE']):?>
			<div class="input-holder">
				<?foreach($arCalc['lines_number']['STRUCTURE'] as $arStructure):?>
				<input placeholder="<?=$arCalc['lines_number']['CAPTION']?>" class="calc_field" data-field-name="form_<?=$arStructure['FIELD_TYPE']?>_<?=$arStructure['ID']?>" data-calc-var="y" type="text"/>
				<?endforeach;?>
			</div>
			<?endif;?>
		</div>
		<div class="form-column">
			<?if($arCalc['power']['STRUCTURE']):?>
			<div class="input-holder">
				<?foreach($arCalc['power']['STRUCTURE'] as $arStructure):?>
				<input placeholder="<?=$arCalc['power']['CAPTION']?>" class="calc_field" data-field-name="form_<?=$arStructure['FIELD_TYPE']?>_<?=$arStructure['ID']?>" data-calc-var="z" type="text"/>
				<?endforeach;?>
			</div>
			<?endif;?>
			<a class="button lightbox-open form-open" <?if (SITE_SERVER_NAME == "elevel.ru"):?>onclick="yaCounter1485305.reachGoal('rasschitat');return true;"<?endif;?> href="#<?=$cntId?>_calculate"><?=($arParams['BUTTON_TITLE'])?$arParams['BUTTON_TITLE']:GetMessage('CALC2_BUTTON_TITLE')?></a>
		</div>
	</div>
</form>
...
<div class="lightbox-holder">
	<div id="<?=$cntId?>_calculate" class="lightbox calculate">
        ...
        <p><?=GetMessage('CALC2_POPUP_NOTE')?></p>
				...
				<?=$arResult["FORM_HEADER"]?>
					...
					foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion)
					{
						...
						if ($arQuestion['STRUCTURE'][0]['FIELD_TYPE'] == 'hidden')
						{
							...
							elseif($FIELD_SID == "producer" || $FIELD_SID == "lines_number" || $FIELD_SID == "power"):
							?>
...
$(function(){	
	$('#<?=$cntId?>_calc_form .button.lightbox-open').on('click',function(){
		...
		if(!$('#<?=$cntId?>_calculate .feedback_form_note').hasClass('success')){
			...
			_res = parseInt((_x * _z)/(0.66) + (_x * _y * _z)/(6.6));
			...
			if(_res == 0 || isNaN(_res))
			{
				...
				$('#<?=$cntId?>_power').html(_z);
				$('#<?=$cntId?>_lines_number').html(_y);
