...
<script>
    var IPOLSDEK_pvz = {
        ...
        cities: <?=CUtil::PhpToJSObject($arResult['Regions'])?>,

        cityId: '<?=$arParams['CITY_ID']?>',

        pvzInputs: [<?=substr($arResult['propAddr'], 0, -1)?>],// inputs 4 PVZ address
        ...
        initCityPVZ: function () { // loading PVZ 4 chosen city
            ...
            //city = $('input[name="address_deliveryCity"]').val();
            //
            var cities = IPOLSDEK_pvz.cities;	//для Московской области
           // alert('ipolSdek CityPVZ - initialized ' + IPOLSDEK_pvz.cities[0]);
            IPOLSDEK_pvz.cityPVZ = {};
            if (IPOLSDEK_pvz.punctMode != 'POSTOMAT') { // Profiler

                for (var city_key in cities) {
                   // alert('cities - ' + cities);
                   // alert('city key - ' + city_key);
                    for (var i in IPOLSDEK_pvz.PVZ[cities[city_key]]) {
                        IPOLSDEK_pvz.cityPVZ[i] = {
                            'Name': (IPOLSDEK_pvz.PVZ[cities[city_key]][i]['Name']) ? IPOLSDEK_pvz.PVZ[cities[city_key]][i]['Name'] : IPOLSDEK_pvz.PVZ[city][i]['Address'],
                            'Address': IPOLSDEK_pvz.PVZ[cities[city_key]][i]['Address'],
                            'WorkTime': IPOLSDEK_pvz.PVZ[cities[city_key]][i]['WorkTime'],
                            'Phone': IPOLSDEK_pvz.PVZ[cities[city_key]][i]['Phone'],
                            'Note': IPOLSDEK_pvz.PVZ[cities[city_key]][i]['Note'],
                            'cX': IPOLSDEK_pvz.PVZ[cities[city_key]][i]['cX'],
                            'cY': IPOLSDEK_pvz.PVZ[cities[city_key]][i]['cY'],
                            'type': 'PVZ'
                        };
                    }
                }
            }
            //alert(IPOLSDEK_pvz.cityPVZ);
        ...
        markChosenPVZ: function (id) {
            ...
            //проставление доставки
            $('input[name="total-delivery"]').val(parseFloat(IPOLSDEK_pvz.prices[ind][0]));
        },
        ...
        Y_markPVZ: function () {
            for (var i in IPOLSDEK_pvz.cityPVZ) {
                ...
                //кастомная кнопка(взята из шаблона order)
                address = IPOLSDEK_pvz.cityPVZ[i].Address.substr(0, IPOLSDEK_pvz.cityPVZ[i].Address.indexOf(',')) + ' ' + IPOLSDEK_pvz.cityPVZ[i].Address.substr(IPOLSDEK_pvz.cityPVZ[i].Address.indexOf(',') + 1).trim();
                baloonHTML += "<div><a data-fancybox-close id='SDEK_button_new' onclick='IPOLSDEK_pvz.chooseAddress(\"" + address + "\")'></a></div>";
                baloonHTML += "</div>";
        ...
        chooseAddress: function (address) {
            ...
            $('input[name="address_point"]').val(address);
            //$('.point_sdek').html(address);
            $('#cdek_delivery_point_button').text(address);
            $('#total-delivery').html($('input[name="total-delivery"]').val());
            var itog = parseFloat($('.total-sum').data('sum')) + parseFloat($('input[name="total-delivery"]').val());
            $('.total-sum span').html(itog.toFixed(2));
        },
        ...
        loadCityCostCourier: function (input) {
            window.courierInput = input;
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    deliveryPrice = parseFloat(data.courier);

                    setDeliveryPrice(window.courierInput, deliveryPrice);
                }
            });
        },
        ...
        setPrices: function () {
            ...
            if (IPOLSDEK_pvz.punctMode != 'ALL') { // Profiler
                ...
                $('.IPOLSDEK_subPunct_detail_PVZ').each(function () {
                    $(this).html(IPOLSDEK_pvz.prices.pickup[0] + " &nbsp;" + IPOLSDEK_pvz.prices.pickup[1]);
                });
        },
...
<div id='SDEK_pvz'>
    <div id='SDEK_title'>
        <div id='SDEK_cityPicker' <?= (count($arResult['Regions']) == 1) ? 'style="visibility:hidden"' : '' ?>>
            ...
            <div>
                <div id='SDEK_cityLabel'>
                    <a id='SDEK_cityName' href='javascript:void(0)'
                       onmouseover='IPOLSDEK_pvz.showCitySel(); return false;'><?= $arResult['city'] ?></a>
                    <div id='SDEK_citySel'>
                        <input type='text' id='SDEK_citySearcher' placeholder='<?= GetMessage("IPOLSDEK_CITYSEARCH") ?>'
                               onkeyup='IPOLSDEK_pvz.getCityName()'/>
                        <? foreach ($arResult['Regions'] as $city) { ?>
                            <a href='javascript:void(0)' <?= ($city == CDeliverySDEK::toUpper($arResult['city'])) ? "style='display:none'" : '' ?>
                               onclick='IPOLSDEK_pvz.chooseCity("<?= $city ?>");return false;'
                               class='SDEK_citySelect'><?= $city ?><br></a>
                        <? } ?>
                    </div>
                </div>
            </div>
        ...
        <div class='SDEK_mark'>
            <div class='SDEK_courierInfo'>
                <strong><?= GetMessage("IPOLSDEK_COURIER") ?></strong><br><?= GetMessage("IPOLSDEK_DELTERM") ?></div>
            <div class='SDEK_courierInfo'>
    ...
    <div id='SDEK_map'></div>
    <div id='SDEK_info'>
        <div id='SDEK_sign'>Список пунктов<?// GetMessage("IPOLSDEK_LABELPVZ") ?></div>
        <div id='SDEK_modController'>
            <? foreach (array('ALL', 'PVZ') as $mode) { // Profiler?>
                <div class='SDEK_mC_block' id='SDEK_mC_<?= $mode ?>'
                     onclick='IPOLSDEK_pvz.switchMode("<?= $mode ?>",true)'><?= GetMessage("IPOLSDEK_TEMPL_$mode") ?></div>
            <? } ?>
            <div style='clear:both'></div>
        </div>
