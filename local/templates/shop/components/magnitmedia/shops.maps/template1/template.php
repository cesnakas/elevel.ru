...
if( $CityName != "Санкт-Петербург" && $CityName != "Екатеринбург" && $CityName && "Новосибирск") {
	if($arAdr['NAME'] == "Москва и Область"){
		$data_city="active";
	}
}
	?>
	<li <?if($i>1):?>class="mobile-hidden"<?endif;?>><a class="<?=$data_city?> tab-link" data-city-id="<?=$arAdr['ID']?>" title="<?=$arAdr['NAME']?>" href="#tab-city-<?=$arAdr['ID']?>"><?=$arAdr['NAME']?></a></li>
	<?
	$i++;
	endforeach?>
