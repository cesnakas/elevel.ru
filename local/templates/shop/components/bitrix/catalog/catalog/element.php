...
$APPLICATION->SetPageProperty("body_class", "product-page");

$this->setFrameMode(true);

//404
$dbItem = CIBlockSection::GetList( array(), array('IBLOCK_ID'=> $arParams['IBLOCK_ID'],'CODE' => $arResult['VARIABLES']['SECTION_CODE']), false, array('DEPTH_LEVEL') );
if($arItem = $dbItem->GetNext()):
	$SectionLevelURL = $arItem['DEPTH_LEVEL'];
endif;
	
$dbElem = CIBlockElement::GetList(array(), array('IBLOCK_ID'=>$arParams['IBLOCK_ID'], 'CODE'=>$arResult['VARIABLES']['ELEMENT_CODE']), false, false, array('IBLOCK_SECTION_ID'));
if($arElem = $dbElem->GetNext()):
	$SectionId = $arElem['IBLOCK_SECTION_ID'];
	
	$dbItem = CIBlockSection::GetList( array(), array('IBLOCK_ID'=> $arParams['IBLOCK_ID'],'ID' => $SectionId), false, array('DEPTH_LEVEL') );
	if($arItem = $dbItem->GetNext()):
		$SectionLevelElem = $arItem['DEPTH_LEVEL'];
	endif;
endif;
/*
if( $SectionLevelURL != $SectionLevelElem ):
