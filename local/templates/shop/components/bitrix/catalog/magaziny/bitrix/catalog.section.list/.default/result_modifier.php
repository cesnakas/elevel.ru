...
$path = explode('/', $APPLICATION->GetCurDir());
if(!empty($path[2])){
    $arResult["CURRENT_SECT"] = $path[2];
} 
else{
    GLOBAL $curSection;  
    foreach($arResult["SECTIONS"] as $ark => $arSect){
        if($arSect["ID"] == $curSection){
            $arResult["CURRENT_SECT"] = $arSect["CODE"];
        }
    }
}
?>