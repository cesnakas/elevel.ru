...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		$arItem['LABEL_VALUE'] = $arParams['TEXT_LABEL_GIFT']?: Loc::getMessage('SGB_TPL_TEXT_LABEL_GIFT_DEFAULT');
...
foreach ($arResult['ITEMS'] as $key => &$arItem)
{
	...
	{
		$arPhotoSmall = CFile::ResizeImageGet($arItem["PREVIEW_PICTURE"]['ID'], array('width'=>292, 'height'=>292), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, Array("name" => "sharpen", "precision" => 0));
		$arItem["PICTURE"] = $arPhotoSmall["src"];
	}
	elseif ($arItem["DETAIL_PICTURE"]["SRC"])
	{
		$arPhotoSmall = CFile::ResizeImageGet($arItem["DETAIL_PICTURE"]['ID'], array('width'=>292, 'height'=>292), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, Array("name" => "sharpen", "precision" => 0));
		$arItem["PICTURE"] = $arPhotoSmall["src"];
	}
