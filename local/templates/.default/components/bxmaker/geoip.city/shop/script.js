...
if (!!window.BXmakerGeoIPCity === false) {
    ...
    (function () {
        ...
        BXmakerGeoIPCityConstructor.prototype.initEvent = function () {
			...
			$(document).on('click', '.js-bxmaker__geoip__city__line a.open', function(){
                //that.popupShow();
            });
			...
			that.popup.on("click", '.block-regions ul li a', function () {
                var item = $(this);
				var region = item.attr('data-region');

				// оставляем отфильтрованные по регионам города
                that.popup.find('.list-cities a.js-bxmaker__geoip__popup-option[data-region!="' + region + '"]').closest("li").hide();
                that.popup.find('.list-cities a.js-bxmaker__geoip__popup-option[data-region="' + region + '"]').closest("li").show();
				
				// скрываем пустые группировки по буквам 
				$('.list-cities > li').each(function(i,elemLetter) {
					
					var empty = true;
					
					$(elemLetter).find("li").each(function(j,elemCity) {
						if ($(elemCity).css('display') !== 'none')
							empty = false;
					});
                        ...
                        if (!!r.response) {
                            if (r.response.count <= 0) {
								...
								for (var i = 0; i < r.response.count; i++) {
									var region_name = r.response.items[i].region;
									
									if (find(regions, region_name) == -1)
										regions.push(region_name);
								}
								regions.sort();
								
								$("div.block-regions").html('<strong class="title">РЕГИОНЫ</strong>	<ul id="new_regions"></ul>');
								
								for (var i = 0; i < regions.length; i++) {
									$("#new_regions").append('<li><a title="' + regions[i] + '" data-region="' + regions[i] + '" href="">' + regions[i] + '</a></li>');
								}
								
								// редактируем города
								var first_letter = r.response.items[0].city.charAt(0);
								
								$("ul.list-cities").html('<li> <strong class="letter text-orange">' + first_letter + '</strong>	<ul id="new_search_cities"></ul> </li>');
								
                                for (var i = 0; i < r.response.count; i++) {
                                    var item = r.response.items[i];

                                    var itemName = (item.city);
                                    var itemHtml = '<li><a class="close-link js-bxmaker__geoip__popup-option" title="' + itemName + '"';

                                    itemHtml += (' data-location="' + item.location + '" ');
                                    itemHtml += (' data-city="' + item.city + '" ');
                                    itemHtml += (' data-city_id="' + item.city_id + '" ');
                                    itemHtml += (' data-id="' + item.city_id + '" ');
                                    ...
                                    $("#new_search_cities").append($(itemHtml));
