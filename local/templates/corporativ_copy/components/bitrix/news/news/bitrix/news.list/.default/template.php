...
?>
<ul class="list-news" id="comp_<?=$bxajaxid?>">
		...
		<li>
			<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
				<?if($arParams["DISPLAY_PICTURE"]!="N" && $arItem["DETAIL_PICTURE"]["SRC"]):?>
					<img class="alignright" src="<?=$arItem["DETAIL_PICTURE"]["SRC"]?>" alt="" />
				<?endif;?>
				...
				<div class="text-block">
					...
					<?if($arParams["DISPLAY_PREVIEW_TEXT"]!="N" && $arItem["PREVIEW_TEXT"]):?>
						<p><?=TruncateText( HTMLToTxt($arItem["PREVIEW_TEXT"], "", array("'<img[^>]*?>'si"), false), 100);?></p>
					<?endif;?>
...
<?if($arResult["NAV_RESULT"]->nEndPage > 1 && $arResult["NAV_RESULT"]->NavPageNomer<$arResult["NAV_RESULT"]->nEndPage):?>
	<div id="btn_<?=$bxajaxid?>" class="button-center">
		...
		<a href="/subscribe/" <?if (SITE_SERVER_NAME == "elevel.ru"):?>onclick="yaCounter1485305.reachGoal('PodpisatsyaNaNovosti');return true;"<?endif;?> class="button margined">Подписаться на новости</a>
	</div>
<?endif?>