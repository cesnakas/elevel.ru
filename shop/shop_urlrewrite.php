...
$select = Array();
$filter = Array("IBLOCK_ID" => CATALOG_ID, "CODE" => $mass[count($mass)]);
$result = CIBlockElement::GetList(Array(), $filter, false, false, $select);
while($items = $result->GetNext()){
    if ($items["DETAIL_PAGE_URL"] != $parse["path"]) {
        LocalRedirect($dirsite.$items["DETAIL_PAGE_URL"],true,301);    
    }    
}
...
<?if($_SESSION["no_url_params"] !== "Y"):
    ...
    $res_el = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("CODE"=>$url[count($url)-2], "IBLOCK_ID" => CATALOG_ID, "ACTIVE" => "Y"), false, false, Array('ID'));
...
if($url[1] == 'shop' && empty($ELEMENT_ID)){
    ...
    $i = 2;
    while($section == 1){
        $arFilter = array("CODE" => $url[$i], "IBLOCK_ID" => CATALOG_ID, "ACTIVE" => "Y");
        $arSelect = array('ID');
    ...
    $res_alt = CIBlockSectionPropertyLink::GetArray(CATALOG_ID, $SECTION_ID);
    ...
    $arFilter = array("SECTION_ID"=>$SECTION_ID, "ACTIVE"=>"Y", "IBLOCK_ID" => CATALOG_ID, "INCLUDE_SUBSECTIONS" => "Y");
    ...
    foreach($ar_el_filter as $key=>$arrr){
        $properties =  CIBlockProperty::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID" => CATALOG_ID, "ID" =>$key));
