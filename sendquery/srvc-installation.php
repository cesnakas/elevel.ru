...
$(document).ready(function() {
    ...
    $('input[name="form_text_655"]').keypress(function (e){
    ...
    $('input[name="form_text_655"]').keyup(function (){
        var phone = $(this).val();
        
        $('input[name="form_text_655"]').not($(this)).val(phone);
    });
    
    $('input[name="form_email_656"]').keyup(function (){
        var email = $(this).val();
        $('input[name="form_email_656"]').not($(this)).val(email);
    });
    
    $('input[name="form_text_654"]').keyup(function (){
        var name = $(this).val();
        $('input[name="form_text_654"]').not($(this)).val(name);
    });
    
    $('textarea[name="form_textarea_657"]').keyup(function (){
        var comm = $(this).val();
        console.log(comm);
        $('textarea[name="form_textarea_657"]').not($(this)).val(comm);
    });
    
    $('form[name="zakaz_installation"]').attr('id','back_call');
    
    $('#suber').click(function() {
        _gaq.push(['_trackEvent', 'sent_electrolab', 'action', 'opt_label', 1]); yaCounter1485305.reachGoal('sent_electrolab');
        NAMEUSER = $('input[name="form_text_663"]').val();
        EMAIL = $('input[name="form_email_665"]').val();
        PHONEUSER = $('input[name="form_text_664"]').val();
        MESSAGES=$('textarea[name="form_textarea_666"]').val();
        textError='';
        error = 0;
...
<h6>Заказать монтажб программирование<img src="/i/pix.gif" alt="" width="18" height="19" id="callback_close"/></h6>
