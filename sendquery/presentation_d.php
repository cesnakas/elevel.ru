...
$(document).ready(function() {
	...
	$('input[name="form_text_157"]').keypress(function (e){
	...
	$('form[name="SIMPLE_FORM_11"]').attr('id','back_call');
	$('form[name="SIMPLE_FORM_11"]').attr('accept-charset','windows-1251');
	$('form#back_call').ajaxForm({
	        success: function(data) {
	            $('.table10').hide();
	            if($("#simul_form").find("div.success-note").length == 0)
	            {
					$("#simul_form").append('<div style="text-align: center; padding: 80px 0px; font-size: 16px; margin-bottom: 20px;" class="success-note">Ваш запрос успешно отправлен!</div>');
				}
	        } 
	    });
	
	$('#suber').click(function() {
		_gaq.push(['_trackEvent', 'sent_simple_d', 'action', 'opt_label', 1]); yaCounter1485305.reachGoal('sent_simple_d');
		NAMEUSER = $('input[name="form_text_156"]').val();
		PHONEUSER = $('input[name="form_text_157"]').val();
		EMAIL = $('input[name="form_email_158"]').val();
...
/*			
		...
		$('form[name="SIMPLE_FORM_11"]').submit();
		} else{
			$('.error').empty();
...
$('input[name="form_file_571"]').attr('size','12');
...
$APPLICATION->SetTitle("Запросить индивидуальную презентацию для дизайнера и архитектора");
?>
