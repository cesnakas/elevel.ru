...
$(document).ready(function() {
	...
	$('input[name="form_text_771"]').keypress(function (e){
		if( e.which!=8 && e.which!=13 && e.which!=0 && (e.which<48 || e.which>57)){
			$(".error").html("Только цифры").show().fadeOut(3000);
            $('#simul_form').css('height','360px');
            setTimeout( function () {$('#simul_form').css('height','327px');}, 3000);  
			return false;
		}
	});
	...
	$('input[name="form_text_771"]').keyup(function (){
        var phone = $(this).val();
        
        $('input[name="form_text_771"]').not($(this)).val(phone);
    });
    
    $('input[name="form_text_773"]').keyup(function (){
        var email = $(this).val();
        $('input[name="form_text_773"]').not($(this)).val(email);
    });
    
    $('input[name="form_text_769"]').keyup(function (){
        var name = $(this).val();
        $('input[name="form_text_769"]').not($(this)).val(name);
    });
    
    $('textarea[name="form_textarea_775"]').keyup(function (){
        var comm = $(this).val();
        console.log(comm);
        $('textarea[name="form_textarea_775"]').not($(this)).val(comm);
    });

    $('form[name="SIMPLE_FORM_7_SGQru"]').attr('id','back_call');
    
    $('#suber').click(function() {
       // _gaq.push(['_trackEvent', 'sent_electroshit', 'action', 'opt_label', 1]); yaCounter1485305.reachGoal('sent_electroshit');
        NAMEUSER = $('input[name="form_text_769"]').val();
        EMAIL = $('input[name="form_text_773"]').val();
        PHONEUSER = $('input[name="form_text_771"]').val();
        MESSAGES=$('textarea[name="form_textarea_775"]').val();
        textError='';
        error = 0;
...
<h6>Заказать щит<img src="/i/pix.gif" alt="" width="18" height="19" id="callback_close"/></h6>
