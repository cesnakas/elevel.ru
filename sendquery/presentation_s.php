...
$(document).ready(function() {
	...
	$('input[name="form_text_161"]').keypress(function (e){
	...
	$('form[name="SIMPLE_FORM_12"]').attr('id','back_call');
	$('form[name="SIMPLE_FORM_12"]').attr('accept-charset','windows-1251');
	...
	$('#suber').click(function() {
		_gaq.push(['_trackEvent', 'sent_simple_s', 'action', 'opt_label', 1]); yaCounter1485305.reachGoal('sent_simple_s');
		NAMEUSER = $('input[name="form_text_160"]').val();
		PHONEUSER = $('input[name="form_text_161"]').val();
		EMAIL = $('input[name="form_email_162"]').val();
...
*/			$('form#back_call').attr("action", "/sendquery/fcallback.php");
			$('form[name="SIMPLE_FORM_12"]').submit();
		} else{
			$('.error').empty();
...
$('input[name="form_file_572"]').attr('size','12');
...
$APPLICATION->SetTitle("Запросить индивидуальную презентацию для строителя и монтажника");
?>
