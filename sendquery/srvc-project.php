...
$(document).ready(function() {
   // $(".table10").append('<tr><td colspan="4" style="padding:0;"><p class="txt8"><span>*</span> Обязательные для заполнения поля.</p><span id="call_close"><img src="/i/btn140f.gif"></span><span id="suber"><img src="/i/btn140g.gif"></span></td></tr>');
    ...
    $('input[name="form_text_645"]').keypress(function (e){
    ...
    $('input[name="form_text_645"]').keyup(function (){
        var phone = $(this).val();
        
        $('input[name="form_text_645"]').not($(this)).val(phone);
    });
    
    $('input[name="form_email_653"]').keyup(function (){
        var email = $(this).val();
        $('input[name="form_email_653"]').not($(this)).val(email);
    });
    
    $('input[name="form_text_644"]').keyup(function (){
        var name = $(this).val();
        $('input[name="form_text_644"]').not($(this)).val(name);
    });
    
    $('textarea[name="form_textarea_647"]').keyup(function (){
        var comm = $(this).val();
        console.log(comm);
        $('textarea[name="form_textarea_647"]').not($(this)).val(comm);
    });
    
    $('form[name="zakaz_project"]').attr('id','back_call');
...
<h6>Заказать проект<img src="/i/pix.gif" alt="" width="18" height="19" id="callback_close"/></h6>
