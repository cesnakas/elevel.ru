...
<?
if($dir=='/contact/psh/'&&($_COOKIE['chek']==0)):?>
<?
$_COOKIE['town_contact']='28213';
$_SESSION['town_contact']='28213';
setcookie ("chek", 1);
?>
<?else:?>
<?
...
<script type="text/javascript">	console.log(<?=SITE_TEMPLATE_PATH?>)</script>
<!--			
<script>
$(document).ready(function() {
    ...
    $('input[name="form_text_678"]').first().keypress(function (e){
    ...
    $('form[name="question_Pushkino"]').attr('id','back_call');
    $('form[name="question_Pushkino"]').attr('accept-charset','windows-1251');
    
    $('#suber-bottom').click(function() {
        //_gaq.push(['_trackEvent', 'sent_project', 'action', 'opt_label', 1]); yaCounter1485305.reachGoal('sent_project');
        NAMEUSER = $('input[name="form_text_677"]').val();
        EMAIL = $('input[name="form_email_679"]').val();
        PHONEUSER = $('input[name="form_text_678"]').val();
        MESSAGES=$('textarea[name="form_textarea_680"]').val();
        textError='';
        error = 0;
        ...
        $.ajax({
            async: true,
            type: "POST",
            url: '/sendquery/test_callback.php',
            dataType: "text",
            data: m_data,
            beforeSend: function() {
            },
    ...
    <div class="bottom-form" style="float: left;"> 
        <div class="bottom-form-title" style="background-color: #f15922; width: 200px; height: 24px; border-radius: 5px; color: #fff; padding-top: 5px; text-align: center;"><p>Отправить запрос Пушкино</p></div>
