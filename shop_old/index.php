...
CModule::IncludeModule("iblock"); // в 3-4 часа ночи возникает ошибка что в этом файле этот модуль не подключен, поэтому подключаю.
?>
...
$res_el = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("CODE"=>$url[count($url)-2], "IBLOCK_ID" => "53", "ACTIVE" => "Y"), false, false, Array('ID'));
...
if($url[1] == 'shop' && empty($ELEMENT_ID)){  
    ...
    $i = 2;
    while($section == 1){
        $arFilter = array("CODE" => $url[$i], "IBLOCK_ID" => "53", "ACTIVE" => "Y");
        $arSelect = array('ID');
