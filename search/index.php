...
$count=15;
	...
	function title($module_id, $param)
	{
		$array = array("iblock_2"=>"новостям", "iblock_6"=>"новинкам", "iblock_7"=>"акциям", "iblock_12"=>"статьям", "iblock_9"=>"вакансиям", "iblock_11"=>"фотогалереи", 
						"iblock_53"=>"каталог-магазину", "main_"=>"остальным страницам", "tags_"=>"тегам страниц", "_"=>"всему сайту");
	...
	if (isset($_GET['q']) || isset($_GET['tags']))
	{
		...
		else 
		{
			...
			else 
			{
				...
				if(!empty($l)){
					echo '<div><h2>Результаты поиска по сайту:</h2></div>';  
					$countSearch = count_search($q, "all");
				}else{
					echo '<div><h2>Результаты поиска по каталогу :</h2></div>';
					$countSearch = count_search($q, "iblock", 53);   
				}

				echo '<form id="filter_cnt_form" method="GET" ><div style="font-style: italic; font-size:13px;">По запросу <span style="color:#F15922;">"'.htmlspecialcharsEx($_GET['q']).'"</span> найдено позиций: <span style="color:#F15922;">'.$countSearch.'</span> </div>
				<br/><label for="filter_cnt_checkbox">
<input type="hidden" name="q" value="'.htmlspecialcharsEx($_GET['q']).'"/>

<input type="checkbox" name="filter" '.(($_GET['filter'] == 'cnt')?'checked="checked"':'').'id="filter_cnt_checkbox" value="cnt" /> в наличии</label>
				</form>';
				
				if(!empty($l)){
					echo '<div style="font-size: 13px; font-style: italic; margin-top: -16px; position: absolute; right: 0; width: 210px;"><a href="'.$_SERVER["REQUEST_URI"].'&l=">Результаты поиска по каталогу</a></div>';
				}else{
					echo '<div style="font-size: 13px; font-style: italic; margin-top: -16px; position: absolute; right: 0; width: 210px;"><a href="'.$_SERVER["REQUEST_URI"].'&l=all">Результаты поиска по сайту</a></div>';
				}
	...
	<?
					...
					<? if ($countSearch>$count):?>
						<a class="more" onclick="ajax_search('iblock','53','<?=$q?>','catalog')" href='#catalog'>Еще результаты</a>
