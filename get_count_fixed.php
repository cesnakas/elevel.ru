...
$flip_array = array_flip($indexed_urls);

//$i = 0;
foreach($urls as $url)
{
	if (isset($flip_array[ str2url($url) ]))
	{
		file_put_contents("elevel_fixed.csv", implode(" ", $redirects[ str2url($url) ]) . "\n", FILE_APPEND);
		$count_finded++;
	}
	else
	{
		file_put_contents("elevel_not_fixed.csv", $url . "\n", FILE_APPEND);
		$count_not_finded++;
	}
