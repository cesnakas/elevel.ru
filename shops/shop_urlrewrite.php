...
$select = Array();
$filter = Array("IBLOCK_ID" => "62", "CODE" => $mass[count($mass)]);
...
$res_el = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("CODE"=>$url[count($url)-2], "IBLOCK_CODE" => "new_catalog123", "ACTIVE" => "Y"), false, false, Array('ID'));
...
if($url[1] == 'shop' && empty($ELEMENT_ID)){
    ...
    while($section == 1){
        ...
        $arFilter = array("CODE" => $url[$i], "IBLOCK_ID" => 62, "ACTIVE" => "Y");//"IBLOCK_CODE" => "new_catalog"
        $arSelect = array('ID');
    ...
    $res_alt = CIBlockSectionPropertyLink::GetArray(62, $SECTION_ID);
    ...
    //if($USER->IsAdmin()){
        $arFilter = array("SECTION_ID"=>$SECTION_ID, "ACTIVE"=>"Y", "IBLOCK_ID" => 62, "INCLUDE_SUBSECTIONS" => "Y");
    ...
    /*else{    
    foreach($fil_prop as $filter_pr){        
        $arFilter = array("!PROPERTY_".$filter_pr=>false, "SECTION_ID"=>$SECTION_ID, "ACTIVE"=>"Y", "IBLOCK_ID" => 62, "INCLUDE_SUBSECTIONS" => "Y");
    ...
    if($USER->IsAdmin()){
        //echo '<pre>'; print_r($ar_el_filter); echo '</pre>';
    }
...
<?
    ...
    while($ob = $res->GetNext())
    {
            $properties = CIBlockProperty::GetList(array('SORT'=>'ASC'), array("IBLOCK_ID" => 62, "NAME"=>$ob["NAME"], "ACTIVE"=>"Y"));
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        if(ar_res !== null && get_cur_page !== "/shop/index.php"){
